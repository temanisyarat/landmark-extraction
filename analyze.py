...
signer_word_mean = {
    s: {w: np.mean(v) for w, v in wd.items()} for s, wd in signer_word_nan.items()
}
...
ax.bar(
    list(var_means.keys()), list(var_means.values()), color=colors, edgecolor="white"
)
...
bp = ax.boxplot(bp_data, tick_labels=SIGNERS, patch_artist=True)
for patch, color in zip(
    bp["boxes"], plt.cm.viridis(np.linspace(0.2, 0.8, len(SIGNERS)))
):
...
bp = ax.boxplot(bp_data, tick_labels=WORDS, patch_artist=True)
...
report_lines.append("|----------|-----------|-------------|----------|")
report_lines.append(
    f"| Pose (x,y,z) | `{pose_nan_arr.mean() * 100:.2f}%` | `{
        np.median(pose_nan_arr) * 100:.2f}%` | `{pose_nan_arr.std() * 100:.2f}%` |"
)
report_lines.append(
    f"| Hands (combined) | `{hands_nan_arr.mean() * 100:.2f}%` | `{
        np.median(hands_nan_arr) * 100:.2f}%` | `{hands_nan_arr.std() * 100:.2f}%` |"
)
report_lines.append(
    f"| Hand 0 | `{hand0_nan_arr.mean() * 100:.2f}%` | `{
        np.median(hand0_nan_arr) * 100:.2f}%` | `{hand0_nan_arr.std() * 100:.2f}%` |"
)
report_lines.append(
    f"| Hand 1 | `{hand1_nan_arr.mean() * 100:.2f}%` | `{
        np.median(hand1_nan_arr) * 100:.2f}%` | `{hand1_nan_arr.std() * 100:.2f}%` |"
)
report_lines.append("")
...
report_lines.append("|--------|-------------|")
report_lines.append(
    "| `distributions.png` | Frame length, pose NaN %, hands NaN % histograms |"
)
report_lines.append(
    "| `heatmap_signer_word.png` | Hand NaN % heatmap across signers × words (orig only) |"
)
report_lines.append("| `variant_nan.png` | Hand NaN % by augmentation variant |")
report_lines.append(
    "| `hand_lr_nan.png` | Left vs right hand detection rate by signer |"
)
report_lines.append(
    "| `frames_per_signer.png` | Frame length distribution per signer |"
)
report_lines.append("| `frames_per_word.png` | Frame length distribution per word |")
report_lines.append(
    "| `pose_visibility.png` | Pose detection confidence distribution |"
)
report_lines.append(
    "| `frames_vs_nan.png` | Frames vs hand NaN% scatter (colored by pose NaN%) |"
)
